...
function Configuration() {
  ...
  const [isMailboxPublic, setIsMailboxPublic] = useState(false);
  const [isPaused, setIsPaused] = useState(false);
  const [isCountPublic, setIsCountPublic] = useState(false);
  ...
  const handleInfoClick = (type: 'mailbox' | 'pause' | 'count') => {
    const dialogContents = {
      mailbox: {
        message: '타인이 내 번호를 검색했을 때의\n 내 우체통 노출 여부를 설정할 수 있어요.',
        subMessage: '우체통 비공개 시, URL을 통해서만 편지를 받을 수 있어요.'
      },
      pause: {
        message: '편지 수신을 일시적으로 정지할 수 있어요.\n정지 유무와 관계 없이 편지는 작성 가능해요.',
        subMessage: '수신 정지 시, 내 홈에서 \‘편지쓰기\' 기능이 비활성화되어요.'
      },
      count: {
        message: '우체통에 노출되고 있는 편지의 수를\n숨길 수 있어요.',
      ...
      <Wrapper>
        <SettingsSection>
          ...
          <SettingItem>
            ...
            <Toggle isOn={isMailboxPublic} onToggle={() => setIsMailboxPublic(!isMailboxPublic)} />
          </SettingItem>
          <SettingItem>
            <SettingLabel>
              수신 일시 정지
            ...
            <Toggle isOn={isPaused} onToggle={() => setIsPaused(!isPaused)} />
          </SettingItem>
          <SettingItem>
            <SettingLabel>
              수신 편지 개수 공개
            ...
            <Toggle isOn={isCountPublic} onToggle={() => setIsCountPublic(!isCountPublic)} />
          </SettingItem>
        </SettingsSection>
